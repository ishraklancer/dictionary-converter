...
SAMETYPESEQUENCE_SUPPORTED_VALUES = [
    u"m",   # pure text
    u"l",   # pure text (locale encoding instead of cp1252, use --input-file-encoding to specify it)
    u"g",   # Pango markup
    ...
    u"h"    # HTML markup
]           # all the above are cp1252 encoded (except "l") and terminated by \0
...
def read(dictionary, args, input_file_paths):
    ...
    def read_ifo(ifo_path, has_syn, args):
        ...
        ifo_unicode = ifo_bytes.decode("cp1252")     # unicode, always cp1252 by spec
        ifo_obj.close()
    ...
    def read_single_file(dictionary, args, input_file_path):
        ...
        while byte_read:
            if byte_read == b"\0":
                ...
                definition = dict_file_bytes[offset_int:(offset_int + size_int)].decode("cp1252")
                headword = headword.decode("cp1252")
                if args.ignore_case:
...
def write(dictionary, args, output_file_path):
    ...
    # This function should be used only on strings that are known to be in encodings where the bytes corresponding to ASCII letters always represent themselves. This includes cp1252 and the ISO-8859-* charsets, but not for instance double-byte encodings like the Windows Codepage 932, where the trailing bytes of double-byte characters include all ASCII letters. If you compare two CP932 strings using this function, you will get false matches.
    #
    # using Python's builtin lower() and sort() by headword
    # should be equivalent for cp1252 encoded dictionaries (and it is fast)
    #
    ...
    for entry_index in dictionary.entries_index_sorted:
        ...
        headword_bytes = entry.headword.encode("cp1252")
        definition_bytes = entry.definition.encode("cp1252")
    ...
    if dictionary.has_synonyms:
        if args.ignore_synonyms:
            ...
            for pair in dict_syns:
                synonym_bytes = pair[0].encode("cp1252")
                index = pair[1]
